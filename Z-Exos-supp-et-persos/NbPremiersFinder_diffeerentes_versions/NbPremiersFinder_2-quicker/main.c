...
int main() {
    ...
    scanf("%d", &nbmax);
    int tablenbs[nbmax];    //tableaux de nombres premiers enregistrés au fur et à mesure.
    //time de début:
    ...
    for (int i = 2; i <= nbmax; ++i) {
        ...
        for (int j = nbnbspremiers; j > 0; j--) { //prend en compte lui-meme et 1.

            //division test
            result = nbinrun % tablenbs[j - 1];
            //printf("\n nbinrun = %d / %d tablenbs[j-1] = %.0f result", nbinrun, tablenbs[j-1] ,result);
            if (result == 0) {
        ...
        if (nbdiviseurs == 0 || nbnbspremiers==0) { //si vaut exactement 2 alors est premier.
            //imprimer le résultat.
            printf("\nNb premier: %d", nbinrun);
            tablenbs[nbnbspremiers] = nbinrun;
            nbnbspremiers++;
